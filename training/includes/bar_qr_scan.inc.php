...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($_POST["bar_qr_input"] !== "") {
        $bar_code = $_POST["bar_qr_input"];
    }
    ...
    if($open_training === "1") {
        ...
        if(isset($bar_code)) {
            ...
            try {
                ...
                //Check if bar code is Registered
                ...
                $stmt->execute();
                $result_query_bar_code = $stmt->fetchAll();
                $count_row_bar_code = $stmt->rowCount();
        
                if($count_row_bar_code < 1) {
                ...
                foreach($result_query_bar_code as $user_data) {
        ...
        try {
            ...
            if($count_attendance_check < 1) {
                header("location: ../attendance.php?error=user_not_in_list");
                exit();
            }
            else {
            }
    
            //Check if the training have already started
